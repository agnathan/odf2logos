...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:function name="test:item-deep-equal" as="xs:boolean">
    ...
    <xsl:choose>
      ...
      <xsl:when
        test="not($item1 instance of node()) and                     not($item2 instance of
        node())">
  ...
  <xsl:function name="test:node-deep-equal" as="xs:boolean">
    ...
    <xsl:choose>
      ...
      <xsl:when
        test="($node1 instance of attribute() and                      $node2 instance of
        attribute()) or                     ($node1 instance of processing-instruction() and
  ...
  <test:tests>
    ...
    <test:test>
      ...
      <test:param name="value" select="QName('http://www.example.com/xslt/unit-test', 'tests')"/>
      <test:expect select="/test:result">
        <test:result select="QName('http://www.example.com/xslt/unit-test', 'tests')"/>
      </test:expect>
    </test:test>
    <test:test>
